...
- `DATABASE_URL` is defined in a committed `.env` (`file:./dev.db`). The relative SQLite path resolves to `prisma/dev.db` for both the Prisma CLI and the runtime client — do not "fix" it to a root-level path.
